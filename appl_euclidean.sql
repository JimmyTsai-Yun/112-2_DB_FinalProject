...
BEGIN
    ...
    INTO short_array
    FROM (SELECT price FROM target_company_stock_prices WHERE company = 'APPL') AS subquery;
    ...
    FOR i IN 1..array_length(companies, 1) LOOP
        ...
        LOOP
            -- 打印結果
            INSERT INTO appl_euclidean_results (company, best_index, min_dis)
...
UPDATE appl_euclidean_results
SET start_time = cs.timestamp
FROM company_stock_prices cs
WHERE appl_euclidean_results.company = cs.company
  AND appl_euclidean_results.best_index = cs.id;

UPDATE appl_euclidean_results
SET end_time = cs.timestamp
FROM company_stock_prices cs
WHERE appl_euclidean_results.company = cs.company
  AND (appl_euclidean_results.best_index+1258) = cs.id;
