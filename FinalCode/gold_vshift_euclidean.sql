DROP TABLE IF EXISTS gold_vsift_results;

CREATE TABLE gold_vsift_results (
    company TEXT,
...
BEGIN
    ...
    INTO short_array
    FROM (SELECT price FROM target_company_stock_prices WHERE company = 'GOLD' ORDER BY timestamp ASC) AS subquery;
    ...
    FOR i IN 1..array_length(companies, 1) LOOP
        ...
        SELECT ARRAY_AGG(price)
        INTO long_array
        FROM (SELECT price FROM company_stock_prices WHERE company = companyname AND timestamp >= '2017-01-01' AND timestamp < '2022-01-01' ORDER BY timestamp ASC) AS subquery;
        ...
        LOOP
            -- 打印結果
            INSERT INTO gold_vsift_results (company, min_dis)
...
SELECT * FROM gold_vsift_results;